...
const data_controller_hooks_1 = require("./data.controller.hooks");
            ...
            for (let index = 0; index < batch.length; index++) {
                ...
                try {
                    ...
                    const existingRecord = duplicateQuery.length > 0
                        ? await Data_1.Data.findOne(record.data?.toLowerCase() === "visa"
...
const getData = async (req, res) => {
    try {
        ...
        const data = await Data_1.Data.find(query)
            .populate("assignedStaff", "name staffId")
            .populate("files")
            .sort(sortObj)
            .skip(skip)
            .limit(limit ? Number(limit) : 0);
        const total = await Data_1.Data.countDocuments(query);
        const pagination = {
        ...
        const updatedData = await Promise.all(data.map(async (record) => {
            ...
            if (referenceNumber) {
                const registeredRef = await Data_1.Data.findOne({
                    $or: [
            ...
            if (mobile) {
                const selfRegistered = await Data_1.Data.findOne({
        ...
        if (status && status.toLowerCase() !== "all") {
            filteredData = updatedData.filter((r) => {
                ...
                if (r.isBulk && status.toLowerCase() === "success") {
                    return (matchesStatus && !(r.isReferenceRegistered && r.isBulkRegistered));
                }
        ...
        if (type === "old") {
            filteredData = updatedData.filter((r) => r.isBulk && r.isReferenceRegistered && !r.isBulkRegistered);
        }
        else if (type === "new") {
            filteredData = updatedData.filter((r) => r.isBulk && !r.isReferenceRegistered && !r.isBulkRegistered);
        }
...
const updateDataStatus = async (req, res) => {
    try {
        ...
        const recordsWithReminders = await Data_1.Data.find({
            _id: { $in: ids },
            reminderDateAndTime: { $ne: null },
        });
        const recordWithoutReminders = await Data_1.Data.find({
            _id: { $in: ids },
            $or: [
        ...
        const updateDataWithoutReminders = {
            status: status,
...
const submitForm = async (req, res) => {
    try {
        const { name, mobile, whatsapp, altMobNumber, preferCountry, preferJobs, job, searchedHouses, gender, dateOfBirth, maritalStatus, religion, education, jobType, monthlyIncome, spokenLanguage, district, city, expectations, createProfileFor, contactPersonName, trackingId, } = req.body;
        if (!name || !mobile) {
            return res.status(400).json({
                success: false,
        ...
        const slNo = await (0, helper_1.generateUniqueSlNo)();
        const profileId = await data_controller_hooks_1.dataControllerHooks.createRegistrationUniqueSerialNumber(form.formType);
        const newData = new Data_1.Data({
            slNo,
            profileId,
            data: form.formType,
...
const updateForm = async (req, res) => {
    try {
        const { name, mobile, whatsapp, altMobNumber, preferCountry, preferJobs, job, searchedHouses, gender, dateOfBirth, maritalStatus, religion, education, jobType, monthlyIncome, spokenLanguage, district, city, expectations, createProfileFor, contactPersonName, step, status, profilePhoto, trackingId, _id, } = req.body;
        ...
        if (altMobNumber !== undefined)
            updateFields.altMobNumber = altMobNumber;
        ...
        const updatedRecord = await Data_1.Data.findByIdAndUpdate(recordIdToUpdate, updateFields, {
            new: true,
            runValidators: true,
        });
...
const updateRow = async (req, res) => {
    try {
        ...
        const updateFields = data_controller_hooks_1.dataControllerHooks.managetRegistrationPaymentUpdate(existingRecord, req.body);
        if (mobile !== undefined)
            updateFields.mobile = mobile;
        if (altMobNumber !== undefined)
            updateFields.altMobNumber = altMobNumber;
        ...
        if (aadharId !== undefined)
            updateFields.aadharId = aadharId;
        const updatedRecord = await Data_1.Data.findByIdAndUpdate(existingRecord._id, updateFields, {
            new: true,
            runValidators: true,
        });
        return res.status(200).json({
...
const getStaffAssignedData = async (req, res) => {
    try {
        ...
        const { page = 1, limit, dataType, status, data: dataFilter, search, sortBy = "createdAt", sortOrder = "desc", startDate, endDate, } = req.query;
        const query = {
            isDeleted: false,
...
const softDeleteData = async (req, res) => {
    try {
        ...
        const softDeleteTargets = await Data_1.Data.find({
            _id: { $in: idsToDelete },
            isDeleted: false,
        });
        const hardDeleteTargets = await Data_1.Data.find({
            _id: { $in: idsToDelete },
            isDeleted: true,
        });
        if (hardDeleteTargets.length > 0) {
            ...
            const result = await Data_1.Data.deleteMany({ _id: { $in: hardDeleteIds } });
        ...
        if (softDeleteTargets.length > 0) {
            ...
            const result = await Data_1.Data.updateMany({ _id: { $in: softDeleteIds } }, { isDeleted: true });
